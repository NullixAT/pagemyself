...
class PageMyselfPageEditor {
  ...
  static async onIframeLoad() {
    ...
    PageMyselfPageEditor.frameTop.find('.pageeditor-address').html(`<a href="${PageMyselfPageEditor.iframeWindow.location.href}" target="_blank" title="__pagemyself_pageeditor_page_open__">${PageMyselfPageEditor.iframeWindow.location.href}</a>`); // update editor bar information on frame load
    ...
    pageLayoutField.setValue(pageData.layout); // inject editor css into website

    $('head link[href*="pageeditor.min.css"]').each(function () {
      PageMyselfPageEditor.iframeHtml.find('head').append($(this).clone());
    });
    PageMyselfPageEditor.iframeHtml.addClass('pageeditor-website'); // insert editor containers
      ...
      PageMyselfPageEditor.iframeHtml.find('.page-blocks').each(function () {
        ...
        new Sortable(this, {
          ...
          'onStart': function onStart() {
            ...
            blockElMap.forEach(function (blockInstance) {//blockInstance.disableEditing()
            });
          },
          'onEnd': function onEnd() {
            PageMyselfPageEditor.iframeHtml.removeAttr('data-sorting');
            blockElMap.forEach(function (blockInstance) {//blockInstance.enableEditing()
            });
          },
      ...
      for (let id in data) {
        ...
        if (row.help) {
          ...
          openHelp.on('click', function () {
            FramelixModal.show({
              bodyContent: FramelixLang.get(row.help)
            });
          });
        }
      }
