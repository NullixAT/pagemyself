...
class MyselfEdit {
  ...
  static initLate() {
    ...
    $(document).on('click', '.myself-open-website-settings', async function () {
      const modal = await FramelixModal.request('post', MyselfEdit.config.websiteSettingsEditUrl, null, null, false, null, {
        maximized: true
      });
    ...
    $(document).on('click', '.myself-open-theme-settings', async function () {
      const modal = await FramelixModal.request('post', MyselfEdit.config.themeSettingsEditUrl, null, null, false, null, {
        maximized: true
      });
      modal.contentContainer.addClass('myself-edit-font');
      modal.destroyed.then(function () {
  ...
  static bindLiveEditableText(frame) {
    ...
    $(frameDoc).on('focusin', '.myself-live-editable-text', function () {
      ...
      config.saveBtn.on('click', async function () {
        ...
        html = html.replace(/<p[\s\S]*?>([\s\S]*?)<\/p>/g, '<div>$1</div>');
        html = html.replace(/<div><br(?:(?!>)[\s\S])*><\/div>/g, '<br/>');
...
class MyselfBlockLayoutEditor {
  ...
  async render() {
    ...
    tabs.addTab('rows', '__myself_blocklayout_pagelayout__', function () {
      ...
      <div class="framelix-spacer"></div>`); // no rows exist, add info
